...
func Map(value string) *list.List {
	valueSplitSign := func(c rune) bool {
		return !unicode.IsLetter(c)
	}

	resList := list.New()
	splits := strings.FieldsFunc(value, valueSplitSign)

	for _, word := range splits {
		resList.PushBack(mapreduce.KeyValue{
			Key:   word,
...
func Reduce(key string, values *list.List) string {
	...
	for e := values.Front(); e != nil; e = e.Next() {
		...
		if err != nil {
			panic("Error: Value string cannot be parsed into a int number!")
		}

		resCount += num
	}

	return strconv.Itoa(resCount)
}
